    ...
    Function Add-PSAutoRunHash {
        ...
        Process {
            $RawAutoRun | ForEach-Object {
                If ($ShowFileHash) {
                $BinaryPath=$Null
                If ($_.ImagePath -and (Test-Path -Path $_.ImagePath -PathType Leaf)) {
                    $BinaryPath = $_.ImagePath
                }
                elseif ($_.Value -and (Test-Path -Path $_.Value -PathType Leaf)) {
                    $BinaryPath = $_.Value
                }
                if ($BinaryPath) {
                    If (Test-Path -Path $BinaryPath -PathType Leaf) {
                        $_ | Add-Member -MemberType NoteProperty -Name MD5 -Value $(
                            (Get-FileHash -Path $BinaryPath -Algorithm MD5).Hash
                        ) -Force -PassThru |
                        Add-Member -MemberType NoteProperty -Name SHA1 -Value $(
                            (Get-FileHash -Path $BinaryPath -Algorithm SHA1).Hash
                        ) -Force -PassThru |
                        Add-Member -MemberType NoteProperty -Name SHA256 -Value $(
                            (Get-FileHash -Path $BinaryPath -Algorithm SHA256).Hash
                        ) -Force -PassThru
                    } else {
                ...
                $BinaryPath = $_.Value
                    ...
                    if ($BinaryPath) {
                        If (Test-Path -Path $BinaryPath -PathType Leaf) {
                            ...
                            $signature = Get-AuthenticodeSignature -FilePath $BinaryPath -ErrorAction Stop
